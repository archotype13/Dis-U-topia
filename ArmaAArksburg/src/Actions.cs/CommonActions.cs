    ...
    public override ActionResult Perform(Entity actor)
    {
        if (actor.Position != null)
        {
            if (actor.Position.Move(_newCords))
                return new SucceededActionResult( Math.Max(Engine.Instance!.GameManager.CurrentLevel!.Grid.GetCell(_newCords).Item2 / (_speed / 100), 5 ) );
...
public class PathMoveAction(Point targetCords, int speed, int maxVisits = -1) : EntityAction // makes the entity pathfind and move towards a target point
{
